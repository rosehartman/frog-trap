...
![Growth rate of the two-patch metapopulation ($\log \lambda_s$ ) versus dispersal to the predator patch with stochastic survivorship and different degrees of spatial autocorrelation and constant (50% predation). The maximum growth rate for each level of spatial autocorrelation occurs at lower dispersal toward the predator patch with increasing correlation. The maximum increase in $\log \lambda_s$ decreases with increasing autocorrelation. ](figures/autocorrellation1.png)

# Life-history scenarios

The difference between the growth rate when dispersers always disperse to the predator-free patch (perfect habitat site selection), and the peak of the dispersal-growth rate curve, can be thought of as the value of having some individuals make imperfect habitat choices. We will refer to this value as $\delta \log \lambda_{sMAX}$.
...
![The gain from optimal dispersal towards the predation patch ($\delta \log \lambda_{sMAX}$) across different life histories.  In the baseline case (juvenile dispersal and predation on juveniles, solid lone), the gain dereases with greater investment in adult survival (and less in juvenile recruitment).  With predation on adults rather than juveniles (dashed line), there is a greater relative gain to be had by optimal dispersion towards the predation patch, again decreasing the value of dispersal with investment in adult survival.  When adults disperse and but juveniles are preyed upon (dotted lines), there is a unimodal relationship.  When adults disperse and are preyed upon (dash-dot line), $\log \lambda_{sMAX} = 0$ in all cases.](figures/lamdiff_total2.png)
